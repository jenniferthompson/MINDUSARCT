...
1. Cumulative incidence curves: ICU discharge, hospital discharge, MV liberation, ICU readmission
...
mental_medians_primary <- pmap_dfr(
  .l = list(
    df = list(dcfd_adjmeds, del_adjmeds, coma_adjmeds),
    outcome_var = c("dcfd_int", "del_int_all", "coma_int_all"),
    mod_obj = list(dcfd_mod, del_mod, coma_mod)
  ),
  .f = add_outcomep
) %>%
...
## -- Tack Two: DCFDs, delirium, coma all in one -------------------------------
...
plot_medians_pub <- function(
  ...
  if(facet_outcome){
    ...
    p <- p +
      labs(
        title = unique(med_df$title_text)
      )
  }
