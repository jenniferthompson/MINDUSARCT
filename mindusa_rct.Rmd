...
screen_ns <- ptstatus_df %>%
  ...
  filter(screened) %>%
  ## Create indicators for each reason for disqualification
  mutate(
    ...
    dq_exc = ifelse(!disqualified, NA, dq_reason == "Exclusion identified"),
  ...
  select(screened, excluded_imm, approached, refused, enrolled, excluded_later,
         disqualified, randomized, dq_nodel, dq_icudc, dq_exc, dq_diedwd,
         matches("^rand\\_")) %>%
...
screen_table <- screen_ns %>%
  ...
  mutate(
    ...
    status_label = case_when(
      ...
      status == "dq_exc"         ~ "Exclusion identified",
...
screen_table %>%
  ...
  group_rows("Reasons for Disqualification", 9, 12) %>%
  group_rows("Organ Failures Present at Randomization", 13, 15)
