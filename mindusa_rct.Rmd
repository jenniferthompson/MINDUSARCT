...
mental_medians_age <- map_dfr(
  ...
  mutate(
    ...
    trt_short = factor(str_sub(trt, 1, 3), levels = c("Pla", "Hal", "Zip")),
...
tte_ratios_age <- pmap_df(.l = tte_args_age, .f = get_ratios_age) %>%
  mutate(
    ...
    em_text = fct_reorder(em_text, age_consent),
    comp.c_short =
      factor(str_sub(comp.c, 1, 3), levels = c("Pla", "Hal", "Zip")),
...
medians_delcoma_pub <- plot_medians_pub(
  med_df = bind_rows(
    ...
    mutate(
      ## Put delirium on lefthand side
      title_text = forcats::fct_rev(factor(title_text))
    ),
  use_xtitle = TRUE
)
...
## management to extend our figures to the very end.
################################################################################

## -- Goal: Data set w/ one record per time + trt, one column per outcome ------
## -- Function to do dataset prep for one outcome ------------------------------
prep_cuminc_df <- function(cuminc_obj, censor_time, cph_mod){
  ## Extract p-value, subdistribution HR, CI for treatment
...
cif_data <- pmap_df(
  ...
  mutate(
    ...
    tte_outcome_2l = glue(
      "{tte_outcome2}\n\nP{ifelse(p_trt < 0.001, '', '=')}{formatp_nejm(p_trt)}"
    ),
    tte_outcome_2l = fct_reorder(tte_outcome_2l, .x = outcome_order),
    tte_outcome_1l = glue(
      "{outcome_label}: ",
      "{tte_outcome2}; P{ifelse(p_trt < 0.001, '', '=')}{formatp_nejm(p_trt)}"
    ),
...
# ) +
#   facet_grid(tte_outcome_2l ~ trt) +
#   geom_ribbon(
#     aes(ymin = lb, ymax = ub), colour = NA, alpha = 0.15
#   ) +
#   geom_step() +
#   scale_color_manual(values = mindusa_trtcols_long, guide = FALSE) +
#   scale_fill_manual(values = mindusa_trtcols_long, guide = FALSE) +
#   scale_x_continuous(name = "Days after Randomization") +
#   scale_y_continuous(
...
## Wrapper function for facet axis breaks (thanks, Jim Hester!)
my_breaks <- function(x) if (max(x) <= 40) seq(0, 30, 5) else seq(0, 90, 15)
...
cif_pub_facet <- ggplot(
  ...
  scale_x_continuous(name = "Days after Randomization", breaks = my_breaks) +
  scale_y_continuous(
    name = "Cumulative Probability",
    limits = c(0, 1),
    breaks = seq(0, 1, 0.2), labels = paste0(seq(0, 1, 0.2)*100, "%")
...
# ## -- Function to create single TTE plot ---------------------------------------
# plot_cif_ind <- function(outcome, subdist_only = TRUE, main_df = cif_data){
#   
#   if(subdist_only){
#     df <- main_df %>% filter(is_subdist & tte_outcome2 == outcome)
#   } else{
#     df <- main_df %>% filter(tte_outcome2 == outcome)
#   }
#   
#   ggplot(data = df, aes(x = time, y = est, color = trt, fill = trt)) +
#     facet_wrap(~ tte_outcome_1l, nrow = 1) +
#     geom_ribbon(
#       aes(ymin = lb, ymax = ub), colour = NA, alpha = 0.15
#     ) +
#     geom_step() +
...
#       breaks = seq(0, floor(max(df$time)), length.out = 7)
#     ) +
#     scale_y_continuous(
#       name = "Cumulative Probability",
#       limits = c(0, 1),
...
# }
#   
# cif_pub_separate <- map(levels(cif_data$tte_outcome), plot_cif_ind)
# multiplot(plotlist = cif_pub_separate, layout = matrix(1:4, ncol = 2))
