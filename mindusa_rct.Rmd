...
tte_ratios_age <- pmap_df(.l = tte_args_age, .f = get_ratios_age) %>%
  mutate(
    outcome_string = case_when(
      ...
      outcome == "icudis_90" ~ "ICU Discharge,\n90 Days",
      outcome == "hospdis_90" ~ "Hospital Discharge,\n90 Days",
      outcome == "mvlib_30" ~ "Liberation from MV,\n30 Days",
      outcome == "readm_90" ~ "ICU Readmission,\n90 Days",
      TRUE ~ "other"
    ),
    p_sep = ifelse(p_int < 0.001, "", "= "),
    outcome_text =
