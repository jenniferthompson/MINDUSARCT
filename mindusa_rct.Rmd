...
mod_rhs_lin <- paste(c(covariate_vars, em_vars), collapse = " + ")
...
plot_medians_pub <- function(
  ...
  p <- p +
    geom_pointrange(
      aes(ymin = lb, ymax = ub),
      color = as.character(palette_colors[["dred"]]), size = 1#, shape = 1
    ) +
    scale_y_continuous(
      limits = c(0, 14),
      breaks = seq(0, 14, 2),
...
medians_dcfd_pub <- plot_medians_pub(
  med_df = dcfd_adjmeds %>%
    mutate(
      title_text = make_outcome_string(dcfd_mod, "Delirium/Coma-Free Days")
    ),
  use_xtitle = FALSE
)
...
medians_delcoma_pub <- plot_medians_pub(
  med_df = bind_rows(
    del_adjmeds %>%
      mutate(title_text = make_outcome_string(del_mod, "Delirium Days")),
    coma_adjmeds %>%
      mutate(title_text = make_outcome_string(coma_mod, "Coma Days"))
  ) %>%
    mutate(
...
[This figure] presents Kaplan-Meier curves by treatment group up to 90 days after randomization. P-values for 30- and 90-day death are calculated using Cox proportional hazards models clustered by study site, and are adjusted for age at study consent; preexisting cognitive impairment (IQCODE), frailty (CSHA Clinical Frailty Score), and comorbidities (Charlson Comorbidities Index); and severity of illness (modified SOFA) and level of arousal (RASS) at randomization. 
...
## Faceted version: perfect except X axis breaks
