...
## -- Modeling/results helper functions ----------------------------------------
...
text_ph_assume <- function(mod_zph){
  ...
  global_phtext <- ifelse(
    global_phtest < 0.05,
    glue("The global test for proportional hazards indicates the assumption is not fully met (p {formatp(global_phtest)}); please see the figures below for further detail."),
    glue("The global test for proportional hazards indicates no major concern (p = {formatp(global_phtest)}); please see the figures below for further detail.")
  )
...
plot_trt_ratios <- function(
  ...
  ggplot(data = ratio_df, aes(y = effect, x = comp.c)) +
    ...
    labs(
      title = glue("Treatment Group vs {outcome_text}"),
...
outcome_vars <- c(
  ...
  "tte_icudis_30", "ftype_icudis_30", ## Time to final, successful ICU discharge
...
## (This does *not* include variables related to liberation from MV, as that
## would introduce artificial missingness)
pts_missany <- model_df %>%
  dplyr::select(-rass_rand, -on_mv_rand24, -tte_mvlib_30, -ftype_mvlib_30) %>%
  miss_case_table() %>%
  filter(n_missing_in_case > 0) %>%
  summarise(
    nmiss = sum_na(n_cases),
    pctmiss = sum_na(percent)
  )
...
gg_miss_var(
  model_df %>%
    dplyr::select(-id, -rass_rand, -on_mv_rand24, -tte_mvlib_30, -ftype_mvlib_30),
  show_pct = TRUE
) +
...
## Unadjusted Analyses{#unadjusted}
...
### 30-Day Time to Event Outcomes

We describe and test for differences in time to death using Kaplan-Meier curves
and the log-rank test, respectively. Our other time-to-event outcomes, ICU and
hospital discharge and liberation from mechanical ventilation, have a competing
risk of death, meaning that patients who die do not have the opportunity to
experience the outcome. Traditional survival analysis assumes that that the
reason for censoring is independent of the outcome; here, that is not the case.
Kaplan-Meier curves would be misleading in these scenarios, because they do not
take this "competing risk" into account. Therefore, we describe together the
cumulative incidences of both the outcome of interest and death, along with
score tests for the difference between groups in the subdistribution of
interest. Generally, the `[insert value]` patients who withdrew in the hospital
and revoked access to PHI are censored at the time of withdrawal; we censor at
31 days anyone who has experienced neither death nor the outcome of interest by
30 days. More details on censoring are provided with each analysis.

## Adjusted Analyses{#adjusted}
...
explain any of the others (resulting in collinearity); see [above](#exploration).
No covariates suggest evidence of too-high collinearity.
...
competing risk. Generally, the `[insert value]` patients who withdrew in the
hospital and revoked access to PHI are censored at the time of withdrawal; we
censor at 31 days anyone who has experienced neither death nor the outcome of
interest by 30 days. More details on censoring are provided with each analysis.
...
in-hospital and 30-day outcomes. From models which would include all `r n_rand` randomized patients (all except liberation from mechanical ventilation),
complete case analysis excludes `r pts_missany %>% pull(nmiss)` (`r rndformat(pts_missany %>% pull(pctmiss), 0)`%).

We do use single imputation for missing mental status on a daily basis, to
reduce potential bias from the `r round(mean(is.na(subset(daily_int_df, study_status == "Intervention")$mental_status_raw)) * 100)`% of patient-days during the
intervention period without sufficient RASS and/or CAM information to determine
mental status. Please see [Notes on Variable Calculation](#dcfds) for further
...
***Note**: We use single imputation to reduce potential bias from the `r round(mean(is.na(subset(daily_int_df, study_status == "Intervention")$mental_status_raw)) * 100)`% of patient-days during the
intervention period without sufficient RASS and/or CAM information to determine
mental status. Please see [Notes on Variable Calculation](#dcfds) for further
rationale.*
...
dcfd_plot <- ggplot(data = ptsummary_df, aes(x = dcfd_int)) +
  ...
  scale_x_continuous(name = "Delirium/Coma-Free Days", breaks = seq(0, 13, 2)) +
...
For some reason the tabset isn't working here. I am utterly confused.
`r emo::ji("shrug")`
...
##### Adjusted Odds Ratios {.tabset}

Odds ratios higher than 1 are favorable, indicating higher odds of more "good"
days alive and free of brain dysfunction.
...
```{r table_dcfd_ors}
table_trt_ratios(ratio_df = dcfd_ors, ratio_type = "Odds")

```

##### Adjusted Medians {.tabset}
...
## -- Use Liu's function (see chunk orm_functions) to calculate conditional ----
## -- median, CI ---------------------------------------------------------------
...
## Function to get value(s) of each covariate
...
## Create data frame with a single row, excluding treatment values
...
## Repeat those values by row; # rows = # different treatments
...
## Finally! Get actual adjusted medians + bounds for each treatment group;
##  set these in a data.frame
dcfd_adjmeds <- quantile.orm(mod = dcfd_mod, new.data = pred_df) %>%
...
## Now we plot them!
ggplot(data = dcfd_adjmeds, aes(y = quantile, x = forcats::fct_rev(trt))) +
  geom_pointrange(
    aes(ymin = lb, ymax = ub),
    shape = 16, size = 1, colour = as.character(palette_colors["dred"])
  ) +
  ...
  labs(
    title = "Adjusted Median DCFDs by Treatment",
    subtitle = glue(
      "P: {formatp(anova(dcfd_mod)['trt', 'P'])}"
    ),
    y = "Adjusted Median (95% Confidence Interval)",
...
```{r table_dcfd_adjmeds}
dcfd_adjmeds_table <- dcfd_adjmeds %>%
  mutate(
    med_ci = glue(
      "{rndformat(quantile)} ",
      "({rndformat(lb)}, ",
  ...
  dplyr::select(trt, med_ci)

dcfd_adjmeds_table %>%
  kable(
    format = "html",
    col.names = c("Treatment", "Adjusted Median (95% CI)"),
    align = c("c", "r")
  ) %>%
  mykablestyle(position = "left")
...
within the 30 days following randomization.

**Censoring and time to event:**

- The `[insert value]` patients who withdrew from the study prior to day 30 with no known death status are censored at the time of withdrawal.
- Patients who survived the entire 30-day period are censored at day 31.
- Patients who died within the 30-day period are assigned a time value of
`time of death - time of randomization`.

### Unadjusted Analysis

Our unadjusted analysis describes survival using Kaplan-Meier curves and tests
for differences among treatment groups in time to death within 30 days using the
log-rank test. All `r n_rand` randomized patients are included.

`[insert general summary of results here]`

```{r death_30_unadj}
## -- Create Surv, survfit objects for time to death within 30 days ------------
surv_death_30 <-
...
## P-value is not stored with logrank object;
##   extract/calculate test statistic, df, P

## df will be the same for all TTE models
df_tte <- length(levels(model_df$trt)) - 1

chisq_death_30 <- logrank_death_30$chisq
pval_death_30 <- 1 - pchisq(chisq_death_30, df = df_tte)

## -- Kaplan-Meier curves for time to death within 30 days ---------------------
km_death_30 <- survminer::ggsurvplot(
  sf_death_30, ## survfit() object
  ## CIs
  conf.int = TRUE, conf.int.alpha = 0.20,
  ## Show N, cumulative events every 5 days; color text by trt
  break.time.by = 5, risk.table = "nrisk_cumevents",
  risk.table.fontsize = 3, risk.table.col = "strata", tables.height = 0.35,
  legend.labs = gsub("trt=", "", names(sf_death_30$strata)),
...
km_death_30$plot <- km_death_30$plot +
  ...
  labs(
    title = "Kaplan-Meier Curve, 30-Day All-Cause Death",
    ## Add log-rank test results to subtitle
    subtitle = glue(
      "Log-rank test for difference between treatments: ",
      "X2, {rndformat(chisq_death_30, 1)}; ",
      "df, {df_tte}; ",
      "P, {formatp(pval_death_30)}"
    )
  ) +
...
## Add labels to nrisk table
km_death_30$table <- km_death_30$table +
  labs(title = "Number at risk (cumulative number of deaths)") +
  xlab("Days after Randomization") +
  scale_y_discrete(
    name = "placeholder text\n",
    labels = paste0("  ", rev(levels(model_df$trt)))
  ) +
  theme(
...
## Printing this plot with just print(km_death_30) is causing a blank page, a la
## this SO post. No great solution found yet. Printing the pieces separately and
## trying to make them line up manually is the best I've got.
## https://stackoverflow.com/questions/42479112/survival-plots-with-survminer-blank-plot
## Seems to be an issue with html_notebook; html_document doesn't have this.
...
```{r death_30_nrisk, fig.width = 7.5, fig.height = 2}
km_death_30$table
...
```{r death_30_hrs, fig.width = 5, fig.height = 3}
...
```{r table_death_30_hrs}
table_trt_ratios(ratio_df = death_30_hrs)
...
We treat death as a competing risk.

**Censoring and time to events:**

- We censor `r nrow(subset(ptsummary_df, ftype_icudis_30 == "Censored" & tte_icudis_30 < 31))` patients prior to the end of the 30-day period; two withdrew from the
study with no access permitted to PHI. One was discharged prior at time 29.x
days, but died on day 30.x; this patient is censored at the time of discharge,
since s/he experienced neither outcome of interest during the time period, but
died prior to the 31 days assigned to all patients who survived the entire 30
days.
- We censor `r nrow(subset(ptsummary_df, ftype_icudis_30 == "Censored" & tte_icudis_30 == 31))` patients at day 31 who survived the entire 30-day period but remained
in the ICU on day 30.
- We assign patients who died within the 30-day period a time value of
`time of death - time of randomization` and a "failure type" of death; we assign
patients who were successfully discharged within the 30-day period a time value
of `time of discharge - time of randomization` and a "failure type" of ICU
discharge.
...
section](#unadjusted) for more details.

`[insert general summary of results here]`

```{r cuminc_icudc_unadj, fig.height = 5}
## Fit cumulative incidence functions, no covariates except treatments
icudis_30_cif_unadj <- with(model_df, {
  cuminc(
    ftime = tte_icudis_30,
    fstatus = ftype_icudis_30,
    group = trt,
    cencode = "Censored"
  )
})
...
icudis_30_tests_unadj <- icudis_30_cif_unadj$Tests %>%
  ## Coerce to tbl_df; as_tibble makes it easy to keep rownames as new variable
  as_tibble(rownames = "subdist") %>%
  glue_data(
    "{subdist}: X2, {rndformat(stat, 2)}; df, {df}; P, {formatp(pv)}"
  )

ggcompetingrisks(icudis_30_cif_unadj, conf.int = TRUE, ggtheme = mindusa_theme()) +
  scale_color_manual(
    name = "",
    values = c(
  ...
  labs(
    title = "Cumulative Incidence of Successful ICU Discharge",
    subtitle = glue(
      "Competing risk of death prior to discharge\n\n",
      "Tests for difference between treatments:\n",
      "{paste(icudis_30_tests_unadj, collapse = '\n')}"
    ),
    x = "Days after Randomization",
    y = "Cumulative Probability",
    caption = '"Successful" is defined as final ICU discharge followed by at least 48 hours alive.'
  )
...
```{r explore_competingrisks}
################################################################################
## Competing risks are new to me, and using splines and wanting group-added-  ##
## last tests makes things difficult. The cmprsk package seems to be standard ##
## for competing risks; the survival package is classic for time-to-event     ##
## outcomes; and the rms package has some really nice features for splines    ##
## and results summaries. Let's see if we can get them all to give us the     ##
## same results using a model with only linear terms!                         ##
################################################################################

## -- Previously defined objects: ----------------------------------------------
## - mod_rhs_lin: "age_consent + charlson_total + iqcode_total_ph + frailty + sofa_mod_imp_rand + trt + arousal_rand"
## - covariate_vars: character vector with all variables included in mod_rhs_lin
## - tte_icudis_30: time to ICU discharge, death, or censoring
## - ftype_icudis_30: can be Censored, ICU Discharge, or Death
## - model_df: data.frame with all variables for all regression analyses

## -- cmprsk package -----------------------------------------------------------
## Prep: crr() requires matrix of covariate values; doesn't take model formulas
## Extract these from model_df, restricting to complete cases
icudis_30_df <- model_df %>%
  dplyr::select(id, one_of(covariate_vars), tte_icudis_30, ftype_icudis_30)
icudis_30_df <- icudis_30_df[complete.cases(icudis_30_df), ]

## Use model.matrix to create a... model matrix; remove the intercept
icudis_30_modmatrix <-
  model.matrix(as.formula(sprintf("~ %s", mod_rhs_lin)), icudis_30_df)[, -1]

# > head(icudis_30_modmatrix)
#   age_consent charlson_total iqcode_total_ph frailty sofa_mod_imp_rand trtB trtC
# 1    48.59411              4          3.0000       4                 9    0    1
# 2    58.63929              4          3.0000       6                 4    1    0
# 3    66.20945              8          3.1250       6                 7    1    0
# 4    48.76386              4          3.1875       3                15    0    0
# 5    49.17454              0          3.0000       2                 6    0    1
# 6    41.06776              3          3.0625       6                12    1    0
#   arousal_randLightly sedated arousal_randNormal arousal_randAgitated
# 1                           1                  0                    0
# 2                           1                  0                    0
# 3                           0                  0                    1
# 4                           1                  0                    0
...
## Fit competing risks regression using cmprsk
icudis_30_crr <- crr(
  ftime = icudis_30_df$tte_icudis_30,     ## failure time variable
  fstatus = icudis_30_df$ftype_icudis_30, ## failure type variable
  cov1 = icudis_30_modmatrix,             ## model matrix created above
  failcode = "ICU Discharge",             ## failure type of *interest*
  cencode = "Censored"                    ## censoring indicator
)

## HRs, CIs for ICU discharge
# > print(summary(icudis_30_crr)$coef, digits = 4)
#                                  coef exp(coef) se(coef)       z p-value
# age_consent                 -0.009663    0.9904 0.003569 -2.7076 6.8e-03
# charlson_total               0.016782    1.0169 0.023418  0.7167 4.7e-01
# iqcode_total_ph              0.082576    1.0861 0.138195  0.5975 5.5e-01
# frailty                     -0.079739    0.9234 0.039671 -2.0100 4.4e-02
# sofa_mod_imp_rand           -0.109252    0.8965 0.016178 -6.7532 1.4e-11
# trtB                         0.032778    1.0333 0.116178  0.2821 7.8e-01
# trtC                        -0.007354    0.9927 0.122361 -0.0601 9.5e-01
# arousal_randLightly sedated  0.089067    1.0932 0.111930  0.7957 4.3e-01
# arousal_randNormal           0.455087    1.5763 0.169115  2.6910 7.1e-03
# arousal_randAgitated         0.256523    1.2924 0.181392  1.4142 1.6e-01

## Makes sense - patients who were normal at baseline have a much higher hazard
## of discharge than patients deeply sedated, for example; HR for higher
## modified SOFA indicates a lower hazard of discharge; etc

## -- survival package ---------------------------------------------------------
## survival::finegray() creates a dataset meant for use with Fine-Gray competing
## risks models. Many thanks to this StackExchange post for making me aware!
## https://stats.stackexchange.com/questions/212312/add-categorical-variable-in-crr-of-package-cmprsk

## Setup: Create a Surv object with our complete case dataset
icudis_30_Surv <- with(icudis_30_df, {
  Surv(
    time = tte_icudis_30,    ## End time variable
    event = ftype_icudis_30, ## Failure type variable
    type = "mstate"          ## Censoring type = multistate (this would be
                             ##   assumed b/c ftype_icudis_30 is a factor, but
                             ##   why not be explicit?)
  )
})

## Create dataset built for Fine-Gray approach, specifying ICU discharge as our
## event of interest
icudis_30_fg <- finegray(
  icudis_30_Surv ~ .,      ## dataset should include Surv object + all variables
  data = icudis_30_df,     ## complete case dataset
  etype = "ICU Discharge", ## event of interest
  ## Don't really need these, but were helpful when I was spot-checking
  id = id,
  count = "fgcount"
)

## Fit model with usual coxph() syntax; only difference is the use of weights,
## created for us by finegray() and included as a variable in icudis_30_fg
icudis_30_coxph <- coxph(
  as.formula(paste("Surv(fgstart, fgstop, fgstatus) ~", mod_rhs_lin)),
  data = icudis_30_fg,
  weights = fgwt
)

## HRs, CIs for ICU discharge
# > print(summary(icudis_30_coxph)$coef, digits = 4)
#                                  coef exp(coef) se(coef)        z  Pr(>|z|)
# age_consent                 -0.009664    0.9904 0.003695 -2.61522 8.917e-03
# charlson_total               0.016788    1.0169 0.022293  0.75307 4.514e-01
# iqcode_total_ph              0.082539    1.0860 0.144711  0.57037 5.684e-01
# frailty                     -0.079729    0.9234 0.040596 -1.96396 4.953e-02
# sofa_mod_imp_rand           -0.109252    0.8965 0.015951 -6.84923 7.425e-12
# trtB                         0.032775    1.0333 0.120221  0.27262 7.851e-01
# trtC                        -0.007329    0.9927 0.122320 -0.05992 9.522e-01
# arousal_randLightly sedated  0.089071    1.0932 0.114187  0.78004 4.354e-01
# arousal_randNormal           0.455061    1.5763 0.181815  2.50287 1.232e-02
# arousal_randAgitated         0.256547    1.2925 0.183423  1.39867 1.619e-01

## -- rms package --------------------------------------------------------------
## rms::cph() is a wrapper around survival::coxph(), with some added
## functionality and consistency with other rms model functions. Therefore we
## should be able to use nearly the same code as coxph, get the same results,
## but in a way we expect from rms.
icudis_30_cph <- cph(
  as.formula(paste("Surv(fgstart, fgstop, fgstatus) ~", mod_rhs_lin)),
  data = icudis_30_fg,
  weights = fgwt
...
## Lots of tweaks to default summary.rms output to get consistent units with
##  other two options
tmp <- summary(
  icudis_30_cph,
  age_consent = c(50, 51),
  charlson_total = c(1, 2),
  iqcode_total_ph = c(3, 4),
  frailty = c(3, 4),
  sofa_mod_imp_rand = c(6, 7),
  arousal_rand = "Deeply sedated"
) %>%
  ## Get summary.rms() output in a format that's easy to compare with others
  as_tibble(rownames = "var") %>%
  dplyr::select(var, Effect, "S.E.", Type) %>%
  set_names(c("var", "coef", "se", "type")) %>%
  ## For whatever reason, summary.rms() returns the numeric columns as matrices.
  ## We need them to be numeric. This is harder than it should be.
  ## mutate_at doesn't accept matrix inputs. SAD FACE. But we have purrr!
  ## This marks the first time I have found a use for purrr vs mutate methods.
  map_if(.p = ~ inherits(., "matrix"), .f = as.numeric) %>%
  ## Using map_if() splits into a list; put it back together
  bind_cols() %>%
  mutate(
    var = ifelse(type == 2, dplyr::lag(var), var),
    exp_coef = ifelse(type == 1, lead(coef), NA)
  ) %>%
  filter(type == 1) %>%
  dplyr::select(var, coef, exp_coef, se)

## Note: summary.rms() doesn't return that many decimal places.

# > print(tmp, digits = 4)
# # A tibble: 10 x 4
#    var                                               coef exp_coef      se
#    <chr>                                            <dbl>    <dbl>   <dbl>
#  1 age_consent                                   -0.00966    0.990 0.00370
#  2 charlson_total                                 0.0168     1.02  0.0223 
#  3 iqcode_total_ph                                0.0825     1.09  0.145  
#  4 frailty                                       -0.0797     0.923 0.0406 
#  5 sofa_mod_imp_rand                             -0.109      0.897 0.0160 
#  6 trt - B:A                                      0.0328     1.03  0.120  
#  7 trt - C:A                                     -0.00733    0.993 0.122  
#  8 arousal_rand - Lightly sedated:Deeply sedated  0.0891     1.09  0.114  
#  9 arousal_rand - Normal:Deeply sedated           0.455      1.58  0.182  
# 10 arousal_rand - Agitated:Deeply sedated         0.257      1.29  0.183

```

#### Model Fitting and Assumptions {.tabset}

We use competing risks regression, with subdistributions for both ICU discharge
and the competing risk of death, to examine the relationship between treatment
and (primarily) ICU discharge. For further details, please see the [Methods section](#adjusted).
...
## -- Create Surv object -------------------------------------------------------
icudis_30_Surv <- with(model_df, {
  Surv(time = tte_icudis_30, event = ftype_icudis_30, type = "mstate")
})
...
icudis_30_fg <- finegray(
  ...
  as.formula(paste("icudis_30_Surv ~", mod_rhs_lin)),
  data = model_df,
  etype = "ICU Discharge",
  id = id,
  count = "fgcount"
...
```{r icudis_30_assume}
## -- Check proportional hazard assumption -------------------------------------
icudis_30_zph <- cox.zph(icudis_30_mod, transform = 'log')

## Extract p-value for global PH assumption; create reproducible statement
icudis_30_phtext <- text_ph_assume(icudis_30_zph)

```

We graphically check our assumption of proportional hazards for the ICU discharge subdistribution, paying particular attention to treatment group and looking for generally flat lines in each panel. `r icudis_30_phtext`

```{r icudis_30_assumeplot}
plot_ph_assume(icudis_30_zph)

```

#### Model Results {.tabset}

We show the hazard ratios for haloperidol and ziprasidone vs placebo. Hazard ratios **higher** than 1 are favorable; for example, an HR of 1.5 for haloperidol vs placebo indicates that, on average, patients in the haloperidol group have 1.5 times the likelihood of successful ICU discharge vs those in the placebo group at any given time point, given that they remain alive and in the ICU.

TREATMENTS ARE ONCE AGAIN COMPLETELY FAKE

##### Visual Form

```{r icudis_30_hrs, fig.width = 5, fig.height = 3.5}
## -- Calculate, plot hazard ratios for both treatments vs placebo -------------
icudis_30_hrs <- trt_ratios(icudis_30_mod)

plot_trt_ratios(
  ratio_df = icudis_30_hrs,
...
```{r table_icudis_30_hrs}
table_trt_ratios(ratio_df = icudis_30_hrs)
