...
ggplot(data = pad_int_trt) +
  aes(x = rass_raw) +
  geom_bar(
    aes(fill = trt), alpha = 0.75, position = "dodge", stat = "count"
  ) +
  scale_x_continuous(
    breaks = seq(-5, 4, 1), name = "Daily RASS Assessments by Research Staff"
  ) +
...
We describe, for each time study drug was temporarily held, the number of doses
for which it was held and whether study drug was ultimately restarted, or not
restarted for one of several reasons.
...
tempholds_info <- bind_rows(tempholds_doses, tempholds_reasons) %>%
...
tempholds_info %>%
  ...
  mykablestyle() %>%
  group_rows("Was drug restarted?", 2, 6)
...
IQCODE (`r sum(is.na(ptsummary_df$iqcode_total_ph))` patients) and level of arousal at randomization (`r sum(is.na(ptsummary_df$rass_rand))` patients) are the only covariates with missing data; the amount is still quite minimal (<2% for each), and these variables are not missing in ways that appear to be related. In the plot below, red indicates missingness and blue indicates a present value. Cases which have both variables missing would be in red in the lower lefthand corner.

(Level of arousal at randomization is more specifically defined as the recorded RASS that is not comatose [ie, > -3] and closest to the time of randomization,
as long as it is within 5 hours before or after randomization.)
