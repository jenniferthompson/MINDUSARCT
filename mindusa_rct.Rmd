...
dcfd_ors_table <- dcfd_ors %>%
  ...
  dplyr::select(comparison, or_ci)

dcfd_ors_table %>%
  kable(
    format = "html",
    col.names = c("Comparison", "Odds Ratio (95% CI)"),
    align = c("l", "r")
  ) %>%
  mykablestyle() %>%
...
km_death_30$plot <- km_death_30$plot +
  ...
  labs(
    title = "Kaplan-Meier Curve, 30-Day All-Cause Mortality",
...
death_30_phtest <- death_30_zph$table["GLOBAL", "p"]
death_30_phtext <- ifelse(
  death_30_phtest < 0.05,
  glue("The global test for proportional hazards indicates the assumption is not fully met (p {formatp(death_30_phtest)}); please see the figures below for further detail."),
  glue("The global test for proportional hazards indicates no major concern (p = {formatp(death_30_phtest)}); please see the figures below for further detail.")
)
...
par(mfrow = c(2, 3))
plot(death_30_zph, col = 'red', lwd = 2)
...
We show the hazard ratios for haloperidol and ziprasidone vs placebo. Hazard ratios **lower** than 1 are favorable, indicating lower likelihood of death at any given time point within 30 days of randomization.
...
ggplot(data = death_30_hrs, aes(y = effect, x = comp.c)) +
  ## Fake row to set up order properly
  geom_point(shape = NA) +
  ## Reference line at 1 (no effect)
  geom_hline(yintercept = 1, linetype = "solid",
  ...
  ## Plot a point for control group
  geom_point(shape = 15, colour = "black", size = 4,
             data = death_30_hrs %>% filter(comp.c == "C")) +
  ## Add ORs, CIs for treatment groups vs control
  geom_pointrange(
    aes(ymin = lcl, ymax = ucl),
    position = position_dodge(width = 0.5),
  ...
  labs(
    title = "Treatment Group vs All-Cause Death",
    subtitle = glue(
      "P: {formatp(anova(death_30_mod)['trt', 'P'])}"
    ),
    y = "Hazard Ratio (95% Confidence Interval)",
    caption = "\nIncludes deaths within 30 days of randomization.\nAdjusted analysis using Cox proportional hazards model."
  ) +
  theme(
    legend.position = "none",
    axis.ticks.y = element_blank(),
...
death_30_table <- death_30_hrs %>%
  ...
  arrange(desc(ref), comp) %>%
  dplyr::select(comparison, hr_ci)

death_30_table %>%
  kable(
    format = "html",
    col.names = c("Comparison", "Hazard Ratio (95% CI)"),
    align = c("l", "r")
  ) %>%
  mykablestyle() %>%
  row_spec(3, color = palette_colors["lgray"])
