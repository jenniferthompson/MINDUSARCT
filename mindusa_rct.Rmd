...
```{r table_del_ors}
table_trt_ratios(ratio_df = del_ors, ratio_type = "Odds")
...
```{r table_coma_ors}
table_trt_ratios(ratio_df = coma_ors, ratio_type = "Odds")
...
```{r table_coma_adjmeds}
coma_adjmeds_table <- coma_adjmeds %>%
  mutate(
    med_ci = glue(
      "{rndformat(quantile)} ",
      "({rndformat(lb)}, ",
