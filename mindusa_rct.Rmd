...
mindusa_theme <- function(base_size = 13){
...
ptsummary_df <- reduce(
  list(
    ptsummary_df, datestrack_df, padsummary_df, dailysummary_df, ptdrug_df
  ),
  left_join,
  by = "id"
)
...
daily_all_df <- reduce(
  list(allptevents_df, paddaily_df, dailydata_df),
  left_join,
...
daily_int_df <- reduce(
  list(randptevents_df, paddaily_df, dailydata_df),
  left_join,
  by = c("id", "redcap_event_name")
)
...
admit_formula_lhs <- "enroll_mv + enroll_nippv + enroll_shock + age_consent + gender + race_cat + ethnicity + english_level + education + iqcode_total_ph + adl_total_ph + faq_total_ph + audit_total_ph_complete + audit_total_ph_partial + frailty_f + insurance + bmi + home_antipsyc + charlson_total + frailty_f + icu_rsn + apache_adm + sofa_adm + cv_sofa_adm_f"
...
inhosp_vars <- c(
  ...
  "on_mv_rand24", "ever_mvlib_rand24", "daysto_mvlib_exp",
  "sepsis", "stroke",
  ...
  "ever_delcoma_int", "delcoma_int_exp", "dcfd_int_nabad", "dcfd_int_nagood",
...
label(inhosp_desc_df$dcfd_int_nabad) <- "Delirium/coma-free days (missing = bad)"
label(inhosp_desc_df$dcfd_int_nagood) <- "Delirium/coma-free days (missing = good)"
...
## Create TTE outcome variables: If patient was not [X], gets hosp_los
ptsummary_df <- ptsummary_df %>%
  mutate(
    tte_icudis = ifelse(!is.na(daysto_icudis), daysto_icudis, hosp_los)
  )

outcome_vars <- c(
  "dcfd_int_nabad", "dcfd_int_nagood",
  "del_int_all", "coma_int_all",
  "death", "hosp_los",             ## Time to death
  "icudis", "tte_icudis",          ## Time to final ICU discharge
  "ever_mvlib", "daysto_mvlib_all" ## Time to liberation from MV
)

covariate_vars <- c(
  "age_consent", "charlson_total", "iqcode_total_ph", "frailty",
  "sofa_mod_imp_rand", "rass_rand", "trt"
)

model_df <- ptsummary_df %>%
  dplyr::select(
    id, study_site, on_mv_rand24, one_of(outcome_vars), one_of(covariate_vars)
  ) %>%
  ## TEMPORARILY TILL RAMEELA AND I DISCUSS: Create absolute value of RASS
  mutate(abs_rass_rand = abs(rass_rand))

## Set datadist to this data.frame
dd <- datadist(model_df)
options(datadist = "dd")

## -- Create sparklines to show distributions of IQCODE, RASS ------------------
barht_iqcode <- model_df %>%
  mutate(iqcode_cuts = cut(iqcode_total_ph, breaks = seq(0, 5, 0.25))) %>%
  group_by(iqcode_cuts) %>%
  summarise(count = n())

spark_iqcode <- sparkline(
  barht_iqcode$count,
  type = 'bar',
  barColor = as.character(palette_colors["dred"]),
  elementId = "spark-iqcode",
  width = 50
)

barht_rass <- model_df %>%
  group_by(abs_rass_rand) %>%
  summarise(count = n())

spark_rass <- sparkline(
  barht_rass$count,
  type = 'bar',
  barColor = as.character(palette_colors["dred"]),
  elementId = "spark-rass",
  width = 35
)
...
notcont_covars <- setdiff(
  gsub("rass_rand", "abs_rass_rand", covariate_vars, fixed = TRUE), cont_covars
)
...
gg_miss_var(
  model_df %>%
    dplyr::select(-id, -on_mv_rand24, -ever_mvlib, -daysto_mvlib_all),
  show_pct = TRUE
) +
...
IQCODE and RASS at randomization have the most missingness, but not in ways that appear to be related. In the plot below, red indicates missingness, blue indicates a present value. Cases which have both variables missing would be in red in the lower lefthand corner.

```{r iqcode_rass_miss}
ggplot(data = model_df) +
  geom_miss_point(aes(x = iqcode_total_ph, y = abs_rass_rand),
...
gg_miss_var(
  model_df %>%
    filter(on_mv_rand24) %>%
    dplyr::select(ever_mvlib, daysto_mvlib_all, one_of(covariate_vars)),
  show_pct = TRUE
) +
  scale_y_continuous(name = "% Missing", limits = c(0, 5)) +
  geom_hline(yintercept = 5, colour = "gray50", linetype = "dotted") +
  labs(
    title = "Outcome and Covariate Missingness for Liberation from MV Model",
    subtitle = "Includes all patients on MV at or within 24h of randomization"
  )
...
To handle days with missing CAM and/or RASS, we calculated delirium/coma-free days (DCFDs) two ways, but use the most conservative approach (missing assumed to be worst outcome) as our primary outcome. Please see [Notes on Variable Calculation](#varcalc) for further rationale.

## Unadjusted Analysis

Our primary unadjusted analysis uses a Kruskal-Wallis test to look for any differences in distribution of DCFDs between the three treatment groups. *(The Kruskal-Wallis test is a nonparametric test analogous to the parametric ANOVA. As seen below, the distribution of DCFDs is quite non-normal; thus a nonparametric test provides more power and reliability.)* As detailed [below](#dcfds), we address days which are missing CAM and/or RASS in two different ways, assuming the worst (primary analysis) and assuming the best (secondary).

```{r dcfds_unadj, results = "hide"}
## -- Test DCFDs by treatment group --------------------------------------------
dcfd_kw_nabad <- with(ptsummary_df, kruskal.test(x = dcfd_int_nabad, g = trt))
dcfd_kw_nagood <- with(ptsummary_df, kruskal.test(x = dcfd_int_nagood, g = trt))
...
## Plan: Plot both DCFD variables at once using facet_grid();
##   need some data management first
dcfd_df <- ptsummary_df %>%
  dplyr::select(id, trt, dcfd_int_nabad, dcfd_int_nagood) %>%
  gather(key = dcfd_type, value = dcfd_value, dcfd_int_nabad:dcfd_int_nagood) %>%
  mutate(
    dcfd_type = factor(
      ifelse(dcfd_type == "dcfd_int_nabad", 1, 2),
      levels = 1:2,
      labels = c("Missing days\nassigned worst value",
                 "Missing days\nassigned best value")
    )
  )

dcfd_plot <- ggplot(data = dcfd_df, aes(x = dcfd_value)) +
  facet_grid(dcfd_type ~ trt) +
  ...
  labs(
    ...
    subtitle = glue(
      ...
      "Missing days assigned worst value: X2, ",
      "{rndformat(dcfd_kw_nabad$statistic, 2)}; ",
      "df, {dcfd_kw_nabad$parameter}; ",
      "P, {formatp(dcfd_kw_nabad$p.value)}",
      "\nMissing days assigned best value: X2, ",
      "{rndformat(dcfd_kw_nagood$statistic, 2)}; ",
      "df, {dcfd_kw_nagood$parameter}; ",
      "P, {formatp(dcfd_kw_nagood$p.value)}"
    ),
    caption = glue(
...
```{r print_dcfds_unadj, fig.width = 7.5, fig.height = 7.5}
dcfd_plot
...
Age, CFS, and Charlson are all allowed to have a nonlinear relationship with our outcome, using restricted cubic splines with `r rcsknots` knots. The distributions of IQCODE `r spark_iqcode` (`r rndformat(mean_na(model_df$iqcode_total_ph == 3) * 100, 0)`% of patients had a score of exactly 3) and the absolute value of RASS at randomization `r spark_rass` have too little variability to allow this flexibility.
...
Hmisc::redun(
  ~ age_consent + charlson_total + frailty + sofa_mod_imp_rand +
    I(iqcode_total_ph) + I(abs_rass_rand) + trt,
  data = model_df,
  nk = rcsknots
)
...
dcfd_mod_nabad <- lrm(
  as.formula(paste("dcfd_int_nabad ~", mod_rhs)),
  data = model_df,
  x = TRUE, y = TRUE
) %>%
...
dcfd_mod_nagood <- lrm(
  as.formula(paste("dcfd_int_nagood ~", mod_rhs)),
  data = model_df,
  x = TRUE, y = TRUE
) %>%
  robcov(cluster = model_df$study_site)
...
JTHelpers::rms_po_assume(
  ...
  labs(title = "PO assumption for DCFDs, missing assigned worst value") +
...
JTHelpers::rms_po_assume(
  ...
  labs(title = "PO assumption for DCFDs, missing assigned best value") +
  theme(axis.text = element_text(size = 8))
...
```{r dcfd_results, fig.width = 3, fig.height = 3}
...
dcfd_ors <- map_df(
  list(dcfd_mod_nabad, dcfd_mod_nagood),
  lrm_comparisons_addoutcome,
  df = model_df, vname = "trt", refVal = "C"
) %>%
  mutate(
    comp.c = fct_rev(factor(comp.c)),
    outcome = factor(
      case_when(
        outcome == "dcfd_int_nabad" ~ 1,
        TRUE                        ~ 0
        ),
      levels = 1:0,
      labels = c(glue("Worst value\nP: {formatp(anova(dcfd_mod_nabad)['trt', 'P'])}"),
                 glue("Best value\nP: {formatp(anova(dcfd_mod_nagood)['trt', 'P'])}"))
    )
  ) %>%
  ## Keep only one row for reference group
  distinct(ref, comp, effect, is.ref, .keep_all = TRUE)

ggplot(data = dcfd_ors, aes(y = effect, x = comp.c, colour = outcome)) +
...
We currently have `r sum(is.na(subset(daily_int_df, study_status == "Intervention")$mental_status))` in-hospital patient-days during the intervention period which do not have enough CAM and/or RASS information to reliably determine mental status. In addition, we have `r sum(is.na(subset(daily_int_df, study_status == "Withdrawn" & study_day < 14)$mental_status))` in-hospital days where the patient had withdrawn from the study and was not assessed.

Therefore, we calculate DCFDs two ways:

1. **Primary**: Assume all missing days are "bad" (have delirium, coma or death)
2. **Secondary**: Assume all missing days are "good" (alive without brain dysfunction)
