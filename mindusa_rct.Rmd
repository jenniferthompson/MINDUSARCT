...
```{r cuminc_icudc_unadj, fig.height = 7}
## -- Create Surv object -------------------------------------------------------
...
## -- Create survfit object, N at risk table -----------------------------------
icudis_30_survfit <- survfit(icudis_30_Surv ~ trt, data = model_df)

icudis_30_sfsum <- summary(icudis_30_survfit, times = seq(0, 30, 5))

## Extract N still at risk, cumulative number of discharges, deaths at each time
icudis_30_risktable <- data.frame(
  trt = str_replace(icudis_30_sfsum$strata, "trt=", ""),
  time = icudis_30_sfsum$time,
  n_risk = icudis_30_sfsum$n.risk[, 3],
  n_icudis = icudis_30_sfsum$n.event[, 1],
  n_death = icudis_30_sfsum$n.event[, 2]
) %>%
  group_by(trt) %>%
  mutate(
    cumul_icudis = cumsum(n_icudis),
    cumul_death = cumsum(n_death),
    risk_string = glue("{n_risk}\n({cumul_icudis}; {cumul_death})")
  )

icudis_30_risk_plot <- ggplot(
  data = icudis_30_risktable,
  aes(x = time, y = 1, colour = trt)
) +
  facet_wrap(~ trt, nrow = 1) +
  ...
  labs(
    title = "Number at risk (cumulative number of ICU discharges; deaths)",
    ylab = " " ## faking it for spacing purposes
  ) +
  theme(plot.title = element_text(size = basetext_size * 0.8),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.text.y = element_text(colour = "white"), ## faking it for spacing
        axis.ticks.y = element_blank(),
        legend.position = "none")
...
icudis_30_tests_unadj <- icudis_30_cif$Tests %>%
...
icudis_30_cif_plot <- ggcompetingrisks(
  icudis_30_cif, conf.int = TRUE, ggtheme = mindusa_theme()
) +
  scale_color_manual(
    name = "",
    values = c(
...
multiplot(
  plotlist = list(icudis_30_cif_plot, icudis_30_risk_plot),
