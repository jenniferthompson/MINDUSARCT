...
cr_mods_age <- map(
  cr_outcome_fgs,
  ~ cph(
    as.formula(paste("Surv(fgstart, fgstop, fgstatus) ~", mod_rhs_age)),
    data = .,
    weights = fgwt,
    x = TRUE, y = TRUE
  )
...
cr_mods_sofa <- map(
  cr_outcome_fgs,
  ~ cph(
    as.formula(paste("Surv(fgstart, fgstop, fgstatus) ~", mod_rhs_sofa)),
    data = .,
    weights = fgwt,
    x = TRUE, y = TRUE
  )
...
cr_mods_iqcode <- map(
  cr_outcome_fgs,
  ~ cph(
    as.formula(paste("Surv(fgstart, fgstop, fgstatus) ~", mod_rhs_iqcode)),
    data = .,
    weights = fgwt,
    x = TRUE, y = TRUE
  )
...
cr_mods_medsurg <- map(
  cr_outcome_fgs,
  ~ cph(
    as.formula(paste("Surv(fgstart, fgstop, fgstatus) ~", mod_rhs_medsurg)),
    data = .,
    weights = fgwt,
    x = TRUE, y = TRUE
  )
...
cr_mods_sepsis <- map(
  cr_outcome_fgs,
  ~ cph(
    as.formula(paste("Surv(fgstart, fgstop, fgstatus) ~", mod_rhs_sepsis)),
    data = .,
    weights = fgwt,
    x = TRUE, y = TRUE
  )
...
plot_medians_pub <- function(
  ...
  if(facet_outcome){
    p <- p +
      ...
      theme(
        strip.text.x = element_text(hjust = 0, size = basetext_size * 1.3),
