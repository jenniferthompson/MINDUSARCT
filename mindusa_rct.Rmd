...
# ## How many assessments are missing status which should have it?
# miss_mental <- sum(
#   is.na(
#     subset(
#       padasmts_int_df,
...
# )
# 
# ## How many *days* in the hospital have no status?
# asmts_day_inhosp <- padasmts_int_df %>%
#   filter(study_day < 14 & study_status %in% c("Intervention", "Withdrawn")) %>%
#   group_by(id, redcap_event_name) %>%
#   summarise(asmts_avail_raw = sum(!is.na(braindys_raw))) %>%
#   ungroup() %>%
#   mutate(has_asmt_raw = asmts_avail_raw > 0)
# 
# ## Caption to include on all mental status charts
# asmts_miss_caption <- glue(
#   "\n\nAmong assessments on in-hospital days, ",
...
We use proportional odds logistic regression to look at the association between
treatment and duration of hyperactive during the intervention period, adjusting
for baseline patient characteristics. Please see [the Methods section](#adjusted) for more details. All `r n_rand_comp` randomized patients
