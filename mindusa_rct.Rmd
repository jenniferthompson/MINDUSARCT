...
rand_table <- left_join(rand_sums, rand_cont, by = "trt") %>%
  ...
  mutate(
    mean_drug_daily_mg = ifelse(
      mean_drug_daily_mg == "NA [NA, NA]\\\n  NaN +/-   NA", "[no values]",
      mean_drug_daily_mg
    )
  ) %>%
  dplyr::select(
    trt, randomized, npct_studydrug, hrs_random_drug, drug_days, drug_doses,
    mean_drug_daily_ml, mean_drug_daily_mg,
  ...
  mutate(
    sort_order = case_when(
      ...
      table_row == "mean_drug_daily_mg" ~ 7,
      table_row == "npct_held" ~ 8,
      table_row == "npct_permdc" ~ 9,
      table_row == "npct_died" ~ 10,
      table_row == "npct_wd" ~ 11,
      table_row == "npct_elig" ~ 12,
      str_detect(table_row, "^npct\\_held\\_") ~ 13,
      str_detect(table_row, "^npct\\_permdc\\_") ~ 14,
      TRUE ~ 15),
    table_row = case_when(
...
rand_table %>%
  ...
  mykablestyle %>%
  group_rows("Randomized Patients", 2, 12) %>%
  group_rows("Reasons Study Drug Temporarily Held", 13, 20) %>%
  group_rows("Reasons Study Drug Permanently Discontinued", 21, 28)
...
> Study staff continued to document dose opportunities after permanent
discontinuation; therefore, patients could have >1 dose in that category in this
table. Note, however, that it is probably more helpful to look at reasons for
permanent discontinuation by *patient* (previous table) rather than by
individual doses. (For example, 24 doses in the placebo group were held due to
coma due to structural brain damage, but these doses all came from three
patients; similarly, the 22 doses discontinued for suspected NMS all came from
one patient.)
