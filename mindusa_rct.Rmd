...
rand_table <- left_join(rand_sums, rand_cont, by = "trt") %>%
  ...
  mutate(
    sort_order = case_when(
      ...
      str_detect(table_row, "^npct\\_held\\_") ~ 17,
      str_detect(table_row, "^npct\\_permdc\\_") ~ 18,
      TRUE ~ 19),
    table_row = case_when(
...
dose_table <- dose_attempts %>%
  ...
  filter(!str_detect(table_row, "^permdc")) %>%
  ## Table formatting
  mutate(
    sort_order = case_when(
...
pts_missany <- model_df %>%
  dplyr::select(
    covariate_vars,
    setdiff(outcome_vars, str_subset(outcome_vars, "^tte|ftype|event"))
  ) %>%
  miss_case_table() %>%
...
pred_df <- rep(list(pred_df_org), length(levels(model_df$trt))) %>%
  ...
  set_names(
    c(
      str_subset(
        str_subset(names(coef(dcfd_mod)), "^[^y>=]"),
        "^[^trt=]"
      ),
      paste0("trt=", c("Haloperidol", "Ziprasidone"))
    )
  )
...
1. Sepsis at ICU admission

Detailed definitions for each of these potential modifiers of treatment effect will be included with each section.
...
consistent regardless of age; for example, though the oldest patients tend to
have the fewest days alive and free of delirium and coma as a group, the median
DCFDs for each treatment group still overlap considerably at each age presented,
indicating no difference in the effect of treatment on this outcome.

One *possible* exception: Among the youngest patients, the ziprasidone group has
fewer days of delirium, but also a slightly elevated hazard ratio for death
compared to the placebo group. This big-picture interpretation is reflected by
the lack of effect modification for the broad outcome of DCFDs, but may be worth
...
mental_medians_age <- map_dfr(
  ...
  mutate(
    ...
    trt_short = factor(
      ifelse(trt == "Placebo", "Plac", str_sub(trt, 1, 3)),
...
mental_table_age <- mental_medians_age %>%
  ...
  mutate(
    outcome_string = str_replace(outcome_string, "\n", " "),
...
tte_ratios_age <- pmap_df(.l = tte_args_age, .f = get_ratios_age) %>%
  mutate(
    ...
    em_text = fct_reorder(em_text, age_consent),
    comp.c_short = factor(
      ifelse(comp.c == "Placebo", "Plac", str_sub(comp.c, 1, 3)),
      levels = c("Plac", "Hal", "Zip")
    ),
...
There are some hints of effect modification; these don't tell a consistent story, and will need to be interpreted in light of their clinical significance. For example, the ziprasidone and placebo groups look quite similar in terms of overall delirium duration regardless of SOI at randomization, but the haloperidol shifts back and forth relative to those groups at various levels of SOI.

Being in the haloperidol group may be a risk factor for a higher 90-day
mortality rate for patients with less severe illness at randomization, but does
not seem to be associated with mortality rates for the most sick patients. Given
the lack of overall association between treatment and mortality, and the lack of
effect modification between SOI and treatment for 30-day mortality, this should
be interpreted with caution, and together with 12-month mortality results when
those are available later in 2018.
...
we see no effect modification from preexisting cognitive impairment and the
association between treatment and any of our primary or secondary outcomes.
...
```{r plot_mental_outcomes_iqcode, fig.height = 6}
...
Aside from coma duration (where any effect modification is clinically minimal),
we see no effect modification from ICU type and the association between
treatment and any of our primary or secondary outcomes.
...
We see no effect modification from sepsis at ICU admission and the association
between treatment and any of our primary or secondary outcomes.
...
[This figure] presents adjusted median values and 95% confidence intervals by
treatment group for days alive and free of delirium and coma (DCFDs); days of delirium; and days of coma during the 14-day intervention period. P-values refer
to the entire association between treatment and the outcome. Medians and
p-values are calculated using proportional odds logistic regression clustered by study site, and are adjusted for age at study consent; preexisting cognitive impairment (IQCODE), frailty (CSHA Clinical Frailty Score), and comorbidities (Charlson Comorbidities Index); and severity of illness (modified SOFA) and level of arousal (RASS) at randomization.
  ...
  med_df <- med_df %>%
    mutate(
      ...
      trt_short = forcats::fct_relabel(
        trt, ~ ifelse(. == "Placebo", "Plac", str_sub(., 1, 3))
      )
    )
...
[This figure] presents Kaplan-Meier curves by treatment group for death up to 90
days after randomization. P-values for 30- and 90-day death refer to the entire
association between treatment and mortality and are calculated using Cox
proportional hazards models clustered by study site. Models are adjusted for age
at study consent; preexisting cognitive impairment (IQCODE), frailty (CSHA
...
[This figure] presents the cumulative incidence probabilities for time-to-event
outcomes. All four outcomes have a competing risk of death prior to the event; ICU readmission and liberation from mechanical ventilation also have a competing risk of ICU discharge prior to the event. *(Full cumulative incidence functions,
including death and ICU discharge and N at risk, and adjusted hazard ratios for each treatment are provided in the online supplement.)* P-values refer to the
overall effect of treatment. They are calculated using the Fine-Gray method for
competing risks (clustered by study site), and adjusted for age at study
...
*These figures are nearly the same as the versions included earlier in this
report, just with the addition of titles, and thus will not be replicated here.*
...
## Adjusted Hazard Ratios, All Time-to-Event Outcomes
