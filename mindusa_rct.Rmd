...
outcome_vars <- c(
  ...
  "tte_mvlib_14", "ftype_mvlib_14"      ## Time to liberation from MV
)
...
pts_missany <- model_df %>%
  dplyr::select(-rass_rand, -on_mv_rand24, -tte_mvlib_14, -ftype_mvlib_14) %>%
  miss_case_table() %>%
...
gg_miss_var(
  model_df %>%
    dplyr::select(-id, -rass_rand, -on_mv_rand24, -tte_mvlib_14, -ftype_mvlib_14),
  show_pct = TRUE
) +
...
icudis_90_risk_plot <-
  cr_risktable_plot(icudis_90_sfsum, event_string = "ICU discharges")
...
favorable for this outcome. `r icudis_90_text[["A"]]`
...
hospdis_90_sfsum <- summary(hospdis_90_survfit, times = seq(0, 30, 5))

hospdis_90_risk_plot <-
  cr_risktable_plot(hospdis_90_sfsum, event_string = "hospital discharges")
...
cr_combine_plots(
  ...
  xmax = 91, time_breaks = seq(0, 90, 5),
  caption_string = '"Successful" is defined as hospital discharge followed by at least 48 hours alive.'
)
...
favorable for this outcome. `r hospdis_90_text[["A"]]`
...
defined as discontinuation of all types of MV followed by at least 48 hours alive and without reinitiation of MV, during the 14-day intervention period. We
treat death during this time as a competing risk.

**Censoring and time to events:**

- We censor at the time of study withdrawal `r with(ptsummary_df, sum(ftype_mvlib_14 == "Censored" & tte_mvlib_14 < 14.01 & hospdis_noinfo, na.rm = TRUE))` patients who withdrew from the study prior to hospital discharge,
with no discharge or death information available.
- We censor at the time of hospital discharge `r with(ptsummary_df, sum_na(ftype_mvlib_14 == "Censored" & tte_mvlib_14 < 14.01 & !hospdis_noinfo))` patients who were discharged without experiencing either death or successful
liberation from MV.
- We censor at day 14.01 `r with(ptsummary_df, sum_na(ftype_mvlib_14 == "Censored" & tte_mvlib_14 == 14.01))` patients who survived the entire 14-day
intervention period but remained hospitalized on day 14.
- We assign the `r with(ptsummary_df, sum_na(ftype_mvlib_14 == "Death"))` patients who died prior to liberation from MV, within the intervention period, a
time value of `time of death - time of randomization` and a "failure type" of
death; we assign the `r with(ptsummary_df, sum_na(ftype_mvlib_14 == "MV Liberation"))` patients who were successfully liberated from MV within the
intervention period a time value of `time of liberation - time of randomization`
...
## -- Create Surv object -------------------------------------------------------
## Will be used for N at risk for CIF, as well as adjusted model fit
mvlib_14_Surv <- with(model_df_mv, {
  Surv(time = tte_mvlib_14, event = ftype_mvlib_14, type = "mstate")
})

## -- Create and summarize survfit object, create N at risk table --------------
mvlib_14_survfit <- survfit(mvlib_14_Surv ~ trt, data = model_df_mv)
mvlib_14_sfsum <- summary(mvlib_14_survfit, times = c(0, 4, 7, 11, 14))

mvlib_14_risk_plot <-
  cr_risktable_plot(mvlib_14_sfsum, event_string = "liberations from MV")

## -- Fit cumulative incidence functions by treatment --------------------------
mvlib_14_cif <- with(model_df_mv, {
  cuminc(
    ftime = tte_mvlib_14,
    fstatus = ftype_mvlib_14,
    group = trt,
    cencode = "Censored"
  )
})
...
## Create strings for test results
mvlib_14_tests_unadj <- cuminc_test(mvlib_14_cif)

## -- Plot CIFs, N at risk -----------------------------------------------------
## Create plot for competing risks
mvlib_14_cif_plot <- cif_plot(
  cuminc_obj = mvlib_14_cif,
  legend_string = "Liberation from MV",
  event_string = "Successful Liberation from MV",
  test_string = mvlib_14_tests_unadj,
  caption_string = '"Successful" is defined as discontinuation of MV, followed by at least 48 hours alive without reinitiation.'
)

## Print combined plot
cr_combine_plots(
  cif = mvlib_14_cif_plot,
  rt = mvlib_14_risk_plot,
  xmax = 15, time_breaks = c(0, 4, 7, 11, 14),
  caption_string = '"Successful" is defined as discontinuation of MV, followed by at least 48 hours alive without reinitiation.'
)
...
## -- Create dataset specifically for Fine-Gray method of competing risks ------
mvlib_14_fg <- finegray(
  ## Use **linear** RHS here; we'll fit spline terms within model formula itself
  as.formula(paste("mvlib_14_Surv ~", mod_rhs_lin)),
  data = model_df_mv,
  etype = "MV Liberation",
  id = id,
  count = "fgcount"
...
```{r mvlib_14_assume}
## -- Check proportional hazard assumption -------------------------------------
mvlib_14_zph <- cox.zph(mvlib_14_mod, transform = 'log')

## Extract p-value for global PH assumption; create reproducible statement
mvlib_14_phtext <- text_ph_assume(mvlib_14_zph)

```

We graphically check our assumption of proportional hazards for the hospital
discharge subdistribution, paying particular attention to treatment group and
looking for generally flat lines in each panel. `r mvlib_14_phtext`

```{r mvlib_14_assumeplot}
plot_ph_assume(mvlib_14_zph)
...
```{r mvlib_14_hrs_prep}
## -- Calculate hazard ratios for both treatments vs placebo -------------------
mvlib_14_hrs <- trt_ratios(mvlib_14_mod)

## Create explanatory text string for subdistribution HRs
mvlib_14_text <-
  hr_example(hr_df = mvlib_14_hrs, outcome = "liberation from MV")

```

We show the subdistribution hazard ratios for hospital discharge for haloperidol
and ziprasidone vs placebo. Subdistribution hazard ratios **higher** than 1 are
favorable for this outcome. `r mvlib_14_text[["A"]]`

TREATMENTS ARE ONCE AGAIN COMPLETELY FAKE

##### Visual Form

```{r mvlib_14_hrs_plot, fig.width = 5, fig.height = 3.5}
mvlib_14_hrs_plot <- plot_trt_ratios(
  ratio_df = mvlib_14_hrs,
...
```{r table_mvlib_14_hrs}
table_trt_ratios(ratio_df = mvlib_14_hrs)
...
```{r mvlib_14_full_results, warning = FALSE}
rms_results_kable(mvlib_14_mod)
