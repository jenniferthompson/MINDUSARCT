...
mental_medians_age <- map_dfr(
  ...
  mutate(
    ...
    outcome_text =
      sprintf("%s\n\nP %s%s", outcome_string, p_sep, formatp(p_int)),
...
```{r pubfig_mental}
# ## -- Combine adj median data from primary analysis of all mental outcomes -----
# ## Prep: Function to add outcome, p-values for treatment to median df
# add_outcomep <- function(df, outcome_var, mod_obj){
#   df %>%
#     mutate(outcome = outcome_var, p_trt = anova(mod_obj)["trt", "P"])
# }
# 
# ## Create big data.frame of all medians, CIs
# mental_medians_primary <- pmap_dfr(
#   .l = list(
#     df = list(
#       dcfd_adjmeds, del_adjmeds, hyperdel_adjmeds, hypodel_adjmeds, coma_adjmeds
#     ),
#     outcome_var = mental_outcome_vars,
#     mod_obj = list(dcfd_mod, del_mod, hyperdel_mod, hypodel_mod, coma_mod)
#   ),
#   .f = add_outcomep
# ) %>%
#   ## Create facet labels: Outcome + p-value for treatment
#   mutate(
#     outcome_text = case_when(
#       outcome == "dcfd_int"         ~ "Delirium/Coma-Free Days",
#       outcome == "del_int_all"      ~ "Delirium (All)",
#       outcome == "hyperdel_int_all" ~ "Hyperactive Delirium",
#       outcome == "hypodel_int_all"  ~ "Hypoactive Delirium",
#       TRUE                          ~ "Coma"
#     ),
#     outcome_sep = ifelse(p_trt < 0.001, " ", " = "),
#     p_trt = formatp(p_trt),
#     
#     ## Order variables for facets, axis labels
#     trt_short = factor(substr(trt, 1, 3), levels = c("Pla", "Hal", "Zip")),
#     trt = factor(trt, levels = c("Placebo", "Haloperidol", "Ziprasidone")),
#     
#     ## facet_text will be a factor ordered by this variable
#     plot_order = case_when(
#       outcome == "dcfd_int"         ~ 1,
#       outcome == "del_int_all"      ~ 2,
#       outcome == "hyperdel_int_all" ~ 3,
#       outcome == "hypodel_int_all"  ~ 4,
#       TRUE                          ~ 5
#     ),
#     facet_text = glue("{outcome_text}\n\nP{outcome_sep}{p_trt}"),
#     facet_text = fct_reorder(facet_text, plot_order)
#   )
# 
# mental_medians_plot_pub <- ggplot(
#   data = mental_medians_primary, aes(x = trt_short, y = quantile)
# ) +
#   facet_wrap(~ facet_text, ncol = 1) +
#   geom_segment(
#     aes(xend = trt_short, y = lb, yend = ub),
#     color = as.character(palette_colors["dred"])
#   ) +
#   geom_point(color = "black", shape = 15) +
#   ## Axes should accurately reflect our intervention period
#   scale_y_continuous(
#     limits = c(0, 14),
#     breaks = seq(0, 14, 2),
...
# 
# ggsave(
#   filename = "mindusa_mental_medians.pdf",
#   mental_medians_plot_pub,
#   ## cairo_pdf instead of regular pdf, because regular pdf ignored the spaces,
#   ## gave what I believe to be an error related to the TrueType-ness of Lato (?)
#   device = cairo_pdf,
#   path = "pubfigures/pdf",
...
# 
# ggsave(
#   filename = "mindusa_mental_medians.png",
#   mental_medians_plot_pub,
#   type = "cairo",
#   path = "pubfigures/png",
#   width = 7, height = 10, units = "in",
...
# )
# 
# ## Tried this with device = pdf
# # Sys.setenv(R_GSCMD = "/usr/bin/gs")
# # embed_fonts("pubfigures/pdf/mindusa_mental_medians.pdf")
# 
...
# 
# ## -- Tack Two: DCFDs, delirium, coma, mortality all in one --------------------
# ## Create separate plots for each
# plot_medians_pub <- function(med_df, mod_obj, title_string){
...
#     geom_pointrange(
#       aes(ymin = lb, ymax = ub), color = as.character(palette_colors[["dred"]])
#     ) +
#     scale_y_continuous(
#       limits = c(0, 14),
#       breaks = seq(0, 14, 2),
...
#       title = title_string,
#       subtitle = glue(
#         'P {ifelse(anova(mod_obj)["trt", "P"] < 0.001, "", "= ")}',
#         '{formatp(anova(mod_obj)["trt", "P"])}'
#       )
#     )
#   
#   return(p)
...
#   med_df = dcfd_adjmeds,
#   mod_obj = dcfd_mod,
#   title_string = "Delirium/Coma-Free Days"
# )
# 
# medians_del_pub <- plot_medians_pub(
#   med_df = del_adjmeds, mod_obj = del_mod, title_string = "Delirium"
# )
# 
# medians_coma_pub <- plot_medians_pub(
#   med_df = coma_adjmeds, mod_obj = coma_mod, title_string = "Coma"
# )
# 
# make_pub_vsn <- function(ggobj, xlims, xbreaks, title_string){
#   ggobj +
#     scale_y_continuous(limits = xlims, breaks = xbreaks) +
#     labs(title = title_string, caption = NULL)
# }
# 
# dcfd_adjmeds_pub <- make_pub_vsn(
#   dcfd_adjmeds_plot,
#   xlims = c(0, 14), xbreaks = seq(0, 14, 2),
#   title_string = "Delirium/Coma-Free Days"
# )
# del_adjmeds_pub <- make_pub_vsn(
#   del_adjmeds_plot,
#   xlims = c(0, 14), xbreaks = seq(0, 14, 2),
#   title_string = "Delirium"
# )
# coma_adjmeds_pub <- make_pub_vsn(
#   coma_adjmeds_plot,
#   xlims = c(0, 14), xbreaks = seq(0, 14, 2),
#   title_string = "Coma"
# )
# death_30_hrs_pub <- make_pub_vsn(
#   death_30_hrs_plot,
#   xlims = c(0.7, 1.5), xbreaks = c(1, seq(0.7, 1.5, 0.2)),
#   title_string = "30-Day Death"
# )
# death_90_hrs_pub <- make_pub_vsn(
#   death_90_hrs_plot,
#   xlims = c(0.7, 1.5), xbreaks = c(1, seq(0.7, 1.5, 0.2)),
#   title_string = "90-Day Death"
# )
# 
# ggsave(
#   filename = "fig2.pdf",
#   multiplot(
#     dcfd_adjmeds_pub, del_adjmeds_pub, coma_adjmeds_pub,
#     death_30_hrs_pub, death_90_hrs_pub,
#     layout = matrix(c(1, 1, 4, 2, 3, 5), byrow = TRUE, nrow = 2)
#   ),
#   path = "pubfigures/pdf",
#   device = cairo_pdf,
