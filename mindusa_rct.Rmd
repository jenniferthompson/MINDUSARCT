...
We summarize below any protocol noncompliance that increased safety risk to the patient, broken down according to a simple categorization scheme followed prospectively during the conduct of the MIND-USA investigation.
...
noncomp_trt <- c(
  map_int(levels(noncompliance_df$trt), ~ sum_na(noncompliance_df$trt == .)),
  sum(is.na(noncompliance_df$trt))
) %>%
  set_names(c(levels(noncompliance_df$trt), "Not randomized"))
...
noncomp_npct <- noncompliance_df %>%
  ...
  mutate(
    ## Will need to update these to actual treatment names
    A = get_npct(num = A, denom = pluck(noncomp_trt, "A")),
    B = get_npct(num = B, denom = pluck(noncomp_trt, "B")),
    C = get_npct(num = C, denom = pluck(noncomp_trt, "C")),
...
- *Coordination* (Coordination of SAT and SBT)
    - Eligible if: On MV and received both an SAT and SBT
...
abcde_table <- map_df(
  ...
  mutate(
    poptext = case_when(
      Variable == "Days Eligible" & element == "1" ~ "On MV and on unit",
      Variable == "Days Compliant" & element == "1" ~
        "Received SAT, or no SAT for safety reason",
      Variable == "Days Eligible" & element == "2" ~ "On MV and on unit",
      Variable == "Days Compliant" & element == "2" ~ "Received SBT, or no SBT for safety reason",
      Variable == "Days Eligible" & element == "3" ~ "On MV, received SAT+SBT",
...
randomization. We treat death during this time as a competing risk.
...
- We censor at the time of hospital discharge `r with(ptsummary_df, sum_na(ftype_mvlib_30 == "Censored" & tte_mvlib_30 < 30.01 & !hospdis_noinfo))` patients who were discharged without experiencing either death or successful
liberation from MV.
...
- We assign the `r with(ptsummary_df, sum_na(ftype_mvlib_30 == "Death"))` patients who died prior to liberation from MV, within 30 days of randomization,
a time value of `time of death - time of randomization` and a "failure type" of
death; we assign the `r with(ptsummary_df, sum_na(ftype_mvlib_30 == "MV Liberation"))` patients who were successfully liberated from MV within 30 days
...
mvlib_30_sfsum <- summary(mvlib_30_survfit, times = c(0, 4, 7, 11, 14))
