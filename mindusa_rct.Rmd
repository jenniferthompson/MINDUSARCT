...
admit_formula_lhs_rand <- paste(
  admit_formula_lhs,
  "+ antipsyc_adm_pr + sofa_imp_rand + cv_sofa_rand + rass_rand"
)
...
```{r baseline_trt}
my_html(
  summaryM(as.formula(paste(admit_formula_lhs_rand, "trt", sep = " ~ ")),
           data = ptsummary_df2, test = TRUE),
...
In response to reviewer queries, we describe all individual RASS assessments
done by our research staff during the 14-day intervention period.

```{r describe_rass}
pad_int_trt <- left_join(pad_int_df, rand_df, by = "id")
label(pad_int_trt$rass_raw) <- "RASS as performed by research staff"

my_html(
  summaryM(rass_raw ~ trt, data = pad_int_trt, overall = TRUE, continuous = 15),
  caption = "RASS Assessments during Intervention Phase"
)
...
About 75% of doses were successfully given in each of the three treatment groups. Pre-dose QTcs (indicated by 12-lead ECG if available, and telemetry
otherwise) were also roughly equivalent between the three groups.
...
dose_table %>%
  ...
  group_rows("Doses Given (% of All Opportunities)", 5, 17) %>%
  group_rows("Doses Temporarily Held (% of All Opportunities)", 18, 25) %>%
  group_rows("Actual RASS for Doses Held due to Oversedation", 26, 32)
...
investigated by our clinical staff. Potentially identifiable information, including dates, times and parenthetical details, is redacted.
...
drug_aes %>%
  mutate(
    ...
    ## Remove PHI from comments
    description = remove_phi(description)
  ) %>%
...
## -- Create tables of K-M estimates at 30, 90 days ----------------------------
kmest_death_30 <- km_estimates(
  sf_overall = survfit(surv_death_30 ~ 1), sf_trt = sf_death_30, timept = 30
)
kmest_death_90 <- km_estimates(
  sf_overall = survfit(surv_death_90 ~ 1), sf_trt = sf_death_90, timept = 90
)
...
kable(
  ...
  align = c("l", rep("r", 7)),
  col.names = c(
    "Treatment", "Randomized", "Still at Risk", "Died", "Censored", "Estimate",
    "Lower 95%", "Upper 95%"
  ),
  caption = "Kaplan-Meier Estimates, Death at 30 Days after Randomization"
) %>%
  mykablestyle()
...
kable(
  ...
  caption = "Kaplan-Meier Estimates, Death at 90 Days after Randomization"
) %>%
  mykablestyle()
...
descriptives <- bind_rows(descrip_continuous, descrip_npct, orshrs) %>%
  ...
  dplyr::select(variable, version, Placebo, Haloperidol, Ziprasidone) %>%
...
kable(
  ...
  format = "html",
  align = c("l", "l", rep("r", 3)),
  col.names = c("Outcome", "Version", "Placebo", "Haloperidol", "Ziprasidone"),
  caption = "Summary of Primary Outcomes"
) %>%
  collapse_rows(columns = 1) %>%
  mykablestyle() %>%
  add_footnote("For adjusted analyses, results are odds or hazard ratios, with placebo as reference.")
...
1. Survival: 90-day KM curve, including results from 30 + 90-day models, + cumulative incidence curves: ICU discharge, hospital discharge, MV liberation, ICU readmission (outcomes of interest only)
...
## 90-Day Survival + Time-to-Event Outcomes

These are combined into a single figure to align with journal requirements.
...
km_death_90_pub <- km_plot_death(
  sf = sf_death_90, lr = logrank_death_90, inc_risktable = FALSE, time = 90
)

km_death_90_pub$plot <- km_death_90_pub$plot +
  labs(
    title = "A: Kaplan-Meier Curve, 90-Day All-Cause Death"#,
  ...
  theme(plot.subtitle = element_blank(), legend.position = c(0.015, 0.03))
...
ggsave(
  filename = "mindusa_tteoutcomes.pdf",
  multiplot(
    km_death_90_pub$plot +
      theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()),
    nrisk_death_90_pub + theme(axis.title.x = element_text(face = "bold")),
...
ggsave(
  filename = "mindusa_tteoutcomes.png",
  multiplot(
    km_death_90_pub$plot +
      theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()),
    nrisk_death_90_pub + theme(axis.title.x = element_text(face = "bold")),
...
```{r combine_tte}
## -- Save combined KM curve, N at risk table, CIF curves for manuscript -------
ggsave(
  filename = "mindusa_kmtte.pdf",
  multiplot(
    plotlist = list(
      km_death_90_pub$plot +
        theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()),
      nrisk_death_90_pub + theme(axis.title.x = element_blank()),
      cif_pub_facet + theme(axis.title.x = element_text(face = "bold"))
    ),
...
Patients are assumed to have normal mental status after being discharged alive
from the hospital. Patients who withdrew from the study, and were therefore no
longer assessed by research staff, have any remaining assessments during their
intervention period considered missing. (Eg, a patient who withdrew on the
afternoon of study day 10, allowed study staff to continue accessing their
medical record, and was known to die on study day 13 would be considered to have
missing assessments on study days 11 & 12.)

Patients who had study drug temporarily or permanently stopped, but did not
withdraw from the study, continued to have their mental status assessed by study
staff, so assessments are still available.
...
The following table describes the documented reasons that RASS and/or CAM
assessments were missing during the intervention phase. (Assessments which are
listed as died and/or discharged might be where a patient had a morning
assessment, and was then discharged prior to the afternoon assessment.)
Descriptions of "other" reasons are listed below (click to unfurl).

```{r reasons_missing}
pad_int_df2 <- pad_int_df
label(pad_int_df2$rass_incomp_rsn) <- "Reason RASS not performed"
label(pad_int_df2$cam_incomp_rsn) <- "Reason CAM-ICU not performed"

my_html(
  summaryM(rass_incomp_rsn + cam_incomp_rsn ~ 1, data = pad_int_df2),
  caption = "Reasons for Missing Assessments during Intervention Phase"
)

```

<details>
<summary>"Other" Reasons Assessments Not Done</summary>
```{r asmt_other_table}
pad_int_df %>%
  ungroup() %>%
  filter(!is.na(rass_incomp_other) | !is.na(cam_incomp_other)) %>%
  dplyr::select(rass_incomp_other, cam_incomp_other) %>%
  kable(
    format = "html",
    col.names = c("RASS Explanation", "CAM-ICU Explanation"),
    align = c("l", "l")
  ) %>%
  mykablestyle
