...
#### Model Fitting, Checks and Assumptions {.tabset}
...
We show the odds ratios (using placebo as reference) and adjusted medians for DCFDs by treatment group. Odds ratios higher than 1 are favorable, indicating higher odds of more "good" days alive and free of brain dysfunction. Medians are adjusted to the median/mode values of all covariates, so that each represents the expected median number of DCFDs for a patient in each treatment group who is otherwise part of a "typical" *(many air quotes)* population. All results are shown visually, with tables included for exact numbers if desired.
...
We explore the association between treatment and time to death from all causes within the 30 days following randomization. All randomized patients are included.

**Censoring and time to event:**

- The `r nrow(subset(ptsummary_df, is.na(death) & !is.na(tte_death_30)))` patients who withdrew from the study prior to day 30 with no known death status are censored at the time of withdrawal.
- Patients who survived the entire 30-day period are censored at day 31.
- Patients who died within the 30-day period are assigned a time value of `time of death - time of randomization`.

### Unadjusted Analysis

Our unadjusted analysis describes survival using Kaplan-Meier curves and tests for differences among treatment groups in time to death within 30 days using the log-rank test.
...
We graphically check our assumption of proportional hazards, paying particular attention to treatment group and looking for generally flat lines in each panel. `r death_30_phtext`
...
We show the hazard ratios for haloperidol and ziprasidone vs placebo. Hazard ratios **lower** than 1 are favorable, indicating lower likelihood of death at any given time point within 30 days of randomization given that the patient has survived up to that point.
...
We explore the relationship between treatment group and successful ICU discharge, defined as the final ICU discharge from the study hospitalization, followed by at least 48 hours alive. Patients who die prior to ICU discharge are obviously unable to experience that outcome. We could censor patients at the time of death; however, traditional survival analysis assumes that censoring is independent of the outcome, which is clearly not the case here. Rather, we use a competing risks survival model to look at the hazard and cumulative incidence of both ICU discharge and death.

**Censoring and time to events:**

- We censor `r nrow(subset(ptsummary_df, ftype_icudis_30 == "Censored" & tte_icudis_30 < 31))` patients prior to the end of the 30-day period; two withdrew from the study with no access permitted to PHI. One was discharged prior at time 29.x days, but died on day 30.x; this patient is censored at the time of discharge, since s/he experienced neither outcome of interest during the time period, but died prior to the 31 days assigned to all patients who survived the entire 30 days.
- We censor `r nrow(subset(ptsummary_df, ftype_icudis_30 == "Censored" & tte_icudis_30 == 31))` patients at day 31 who survived the entire 30-day period but remained in the ICU on day 30.
- We assign patients who died within the 30-day period a time value of `time of death - time of randomization` and a "failure type" of death; we assign patients who were successfully discharged within the 30-day period a time value of `time of discharge - time of randomization` and a "failure type" of ICU discharge.

### Unadjusted Analyses

A simple Kaplan-Meier curve would be misleading in the case of competing risks, because it does not take into account patients who died. Rather, we present cumulative incidence plots by treatment group for both ICU discharge and death, unadjusted for any covariates.
