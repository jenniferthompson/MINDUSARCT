...
cr_risktable_plot <- function(sf_sum, event_string){
  ...
  ggplot(data = df, aes(x = time, y = 1, colour = trt)) +
    ...
    geom_text(aes(label = risk_string), size = basetext_size * 0.2) +
    scale_x_continuous(
...
To save space, reasons for "other" exclusions are collapsed. Click on `Details` below to view them. (These explanations are taken directly from our database; potentially identifiable information, including dates, times and parenthetical
details, have been redacted to preserve patient privacy.)
...
were on MV when randomized, or were placed on MV within the 24 hours following
randomization.
...
and in the ICU. Study staff continued to document dose opportunities after permanent discontinuation; therefore, patients could have >1 dose in that category in this table.
...
outcome_vars <- c(
  ...
  "tte_icudis_30", "ftype_icudis_30",   ## Time to final, succ. ICU discharge
  "tte_hospdis_30", "ftype_hospdis_30", ## Time to hospital discharge
...
30.01 days anyone who has experienced neither death nor the outcome of interest
by 30 days. More details on censoring are provided with each analysis.
...
- We censor `r with(ptsummary_df, sum(ftype_icudis_30 == "Censored" & tte_icudis_30 < 30.01))` patients prior to the end of the 30-day period; `r with(ptsummary_df, sum(ftype_icudis_30 == "Censored" & tte_icudis_30 < 30.01 & hospdis_noinfo))` withdrew from the study prior to hospital discharge, with no
discharge or death information available, and are therefore censored at the time of study withdrawal. One patient was discharged from the ICU on day 29.9 and died at day 30.7, so that s/he did experienced neither event during the specified time period; this patient is censored at the time of ICU discharge.
- We censor `r with(ptsummary_df, sum(ftype_icudis_30 == "Censored" & tte_icudis_30 == 30.01))` patients at day 30.01 who survived the entire 30-day
period but remained in the ICU on day 30.
...
```{r cuminc_icudis_unadj, warning = FALSE, fig.height = 7}
## -- Create Surv object -------------------------------------------------------
...
icudis_30_survfit <- survfit(icudis_30_Surv ~ trt, data = model_df)
icudis_30_sfsum <- summary(icudis_30_survfit, times = seq(0, 30, 5))

icudis_30_risk_plot <-
  cr_risktable_plot(icudis_30_sfsum, event_string = "ICU discharges")

## -- Fit cumulative incidence functions by treatment --------------------------
icudis_30_cif <- with(model_df, {
  cuminc(
    ftime = tte_icudis_30,
...
icudis_30_tests_unadj <- cuminc_test(icudis_30_cif)

## -- Plot CIFs, N at risk -----------------------------------------------------
## Create plot for competing risks
icudis_30_cif_plot <- cif_plot(
  cuminc_obj = icudis_30_cif,
  legend_string = "ICU Discharge",
  event_string = "Successful ICU Discharge",
  test_string = icudis_30_tests_unadj,
  caption_string = '"Successful" is defined as final ICU discharge followed by at least 48 hours alive.'
)

## Print combined plot
cr_combine_plots(
  cif = icudis_30_cif_plot,
  rt = icudis_30_risk_plot,
  xmax = 31, time_breaks = seq(0, 30, 5),
  caption_string = '"Successful" is defined as final ICU discharge followed by at least 48 hours alive.'
)
...
- We censor `r with(ptsummary_df, sum(ftype_hospdis_30 == "Censored" & tte_hospdis_30 < 30.01))` patients prior to the end of the 30-day period; these
patients withdrew from the study prior to hospital discharge, with no discharge
or death information available, and are therefore censored at the time of study withdrawal.
- We censor `r with(ptsummary_df, sum(ftype_hospdis_30 == "Censored" & tte_hospdis_30 == 30.01))` patients at day 30.01 who survived the entire 30-day
period but remained hospitalized on day 30.
- We assign the `r with(ptsummary_df, sum(ftype_hospdis_30 == "Death"))` patients who died prior to ICU discharge within the 30-day period a time value
of `time of death - time of randomization` and a "failure type" of death; we
assign the `r with(ptsummary_df, sum(ftype_hospdis_30 == "Hospital Discharge"))`
...
```{r cuminc_hospdis_unadj, warning = FALSE, fig.height = 7}
## -- Create Surv object -------------------------------------------------------
## Will be used for N at risk for CIF, as well as adjusted model fit
hospdis_30_Surv <- with(model_df, {
  Surv(time = tte_hospdis_30, event = ftype_hospdis_30, type = "mstate")
})

## -- Create and summarize survfit object, create N at risk table --------------
hospdis_30_survfit <- survfit(hospdis_30_Surv ~ trt, data = model_df)
hospdis_30_sfsum <- summary(hospdis_30_survfit, times = seq(0, 30, 5))

hospdis_30_risk_plot <-
  cr_risktable_plot(hospdis_30_sfsum, event_string = "hospital discharges")

## -- Fit cumulative incidence functions by treatment --------------------------
hospdis_30_cif <- with(model_df, {
  cuminc(
    ftime = tte_hospdis_30,
    fstatus = ftype_hospdis_30,
    group = trt,
    cencode = "Censored"
  )
})
...
## Create strings for test results
hospdis_30_tests_unadj <- cuminc_test(hospdis_30_cif)

## -- Plot CIFs, N at risk -----------------------------------------------------
## Create plot for competing risks
hospdis_30_cif_plot <- cif_plot(
  cuminc_obj = hospdis_30_cif,
  legend_string = "Hospital Discharge",
  event_string = "Successful Hospital Discharge",
  test_string = hospdis_30_tests_unadj,
  caption_string = '"Successful" is defined as hospital discharge followed by at least 48 hours alive.'
)

## Print combined plot
cr_combine_plots(
  cif = hospdis_30_cif_plot,
  rt = hospdis_30_risk_plot,
  xmax = 31, time_breaks = seq(0, 30, 5),
  caption_string = '"Successful" is defined as hospital discharge followed by at least 48 hours alive.'
)
...
## -- Create dataset specifically for Fine-Gray method of competing risks ------
hospdis_30_fg <- finegray(
  ## Use **linear** RHS here; we'll fit spline terms within model formula itself
  as.formula(paste("hospdis_30_Surv ~", mod_rhs_lin)),
  data = model_df,
...
```{r hospdis_30_assume}
## -- Check proportional hazard assumption -------------------------------------
hospdis_30_zph <- cox.zph(hospdis_30_mod, transform = 'log')

## Extract p-value for global PH assumption; create reproducible statement
hospdis_30_phtext <- text_ph_assume(hospdis_30_zph)

```

We graphically check our assumption of proportional hazards for the hospital
discharge subdistribution, paying particular attention to treatment group and
looking for generally flat lines in each panel. `r hospdis_30_phtext`

```{r hospdis_30_assumeplot}
plot_ph_assume(hospdis_30_zph)

```

#### Model Results {.tabset}

```{r hospdis_30_hrs_prep}
## -- Calculate hazard ratios for both treatments vs placebo -------------------
hospdis_30_hrs <- trt_ratios(hospdis_30_mod)

## Create explanatory text string for subdistribution HRs
hospdis_30_text <-
  hr_example(hr_df = hospdis_30_hrs, outcome = "hospital discharge")
...
favorable for this outcome. `r hospdis_30_text[["A"]]`

TREATMENTS ARE ONCE AGAIN COMPLETELY FAKE

##### Visual Form

```{r hospdis_30_hrs_plot, fig.width = 5, fig.height = 3.5}
hospdis_30_hrs_plot <- plot_trt_ratios(
  ratio_df = hospdis_30_hrs,
  outcome_text = "Successful Hospital Discharge",
  extra_text = glue(
    "\n\n",
    '"Successful": hospital discharge within 30 days of randomization, ',
    "followed by at least 48 hours alive.\n",
    "Patients who withdrew & revoked access to PHI ",
    "are censored on the day of withdrawal.\n",
...
```{r table_hospdis_30_hrs}
table_trt_ratios(ratio_df = hospdis_30_hrs)
...
```{r hospdis_30_full_results, warning = FALSE}
rms_results_kable(hospdis_30_mod)
...
defined as discontinuation of all types of MV followed by at least 48 hours alive and without reinitiation of MV. We treat death as a competing risk.

**Censoring and time to events:**

- We censor `r with(ptsummary_df, sum(ftype_mvlib_30 == "Censored" & tte_mvlib_30 < 30.01 & hospdis_noinfo, na.rm = TRUE))` patients at the time of
study withdrawal who withdrew from the study prior to hospital discharge,
with no discharge or death information available.
- We censor `r with(ptsummary_df, sum_na(ftype_mvlib_30 == "Censored" & tte_mvlib_30 < 30.01 & !hospdis_noinfo))` patients at the end of
hospitalization who were discharged without experiencing either death or
successful liberation from MV.
- We censor `r with(ptsummary_df, sum_na(ftype_mvlib_30 == "Censored" & tte_mvlib_30 == 30.01))` patients at day 30.01 who survived the entire 30-day
period but remained hospitalized on day 30.
- We assign the `r with(ptsummary_df, sum_na(ftype_mvlib_30 == "Death"))` patients who died prior to liberation from MV within the 30-day period a time
value of `time of death - time of randomization` and a "failure type" of death;
we assign the `r with(ptsummary_df, sum_na(ftype_mvlib_30 == "MV Liberation"))`
patients who were successfully liberated from MV within the 30-day period a time
value of `time of liberation - time of randomization` and a "failure type" of
liberation from MV.
...
```{r cuminc_mvlib_unadj, warning = FALSE, fig.height = 7}
...
mvlib_30_sfsum <- summary(mvlib_30_survfit, times = seq(0, 30, 5))

mvlib_30_risk_plot <-
  cr_risktable_plot(mvlib_30_sfsum, event_string = "liberations from MV")

## -- Fit cumulative incidence functions by treatment --------------------------
mvlib_30_cif <- with(model_df, {
  cuminc(
    ftime = tte_mvlib_30,
...
We show the subdistribution hazard ratios for hospital discharge for haloperidol
and ziprasidone vs placebo. Subdistribution hazard ratios **higher** than 1 are
favorable for this outcome. `r mvlib_30_text[["A"]]`
...
mvlib_30_hrs_plot <- plot_trt_ratios(
  ...
  extra_text = glue(
    "\n\n",
    '"Successful": liberation from MV within 30 days of randomization, ',
