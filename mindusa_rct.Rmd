...
interest. Generally, the `r with(ptsummary_df, sum(studywd_ih == "Yes" & hospdis_noinfo))` patients who withdrew in the hospital with no discharge or
...
## Now we plot them!
dcfd_adjmeds_plot <- ggplot(
  data = dcfd_adjmeds, aes(y = quantile, x = forcats::fct_rev(trt))
) +
  geom_pointrange(
    aes(ymin = lb, ymax = ub),
    shape = 16, size = 1, colour = as.character(palette_colors["dred"])
...
*collapsed to save space; click Details below to view*
...
- The `r with(ptsummary_df, sum(!hospdis_noinfo & !event_death_30 & tte_death_30 == 31))` patients who survived the entire 30-day period are censored at day 31.
- The `r with(ptsummary_df, sum(!hospdis_noinfo & event_death_30 & tte_death_30 < 31))` patients who died within the 30-day period are assigned a time value of
...
survived up to that point. `r death_30_text[["A"]]`
...
- We censor `r with(ptsummary_df, sum(ftype_icudis_30 == "Censored" & tte_icudis_30 < 31))` patients prior to the end of the 30-day period; `r with(ptsummary_df, sum(ftype_icudis_30 == "Censored" & tte_icudis_30 < 31 & hospdis_noinfo))` withdrew from the study prior to hospital discharge, with no
discharge or death information available. One was discharged prior to death at
time 29.x days, but died on day 30.x; this patient is censored at the time of
discharge, since s/he experienced neither outcome of interest during the time
period, but died prior to the 31 days assigned to all patients who survived the
entire 30 days.
- We censor `r with(ptsummary_df, sum(ftype_icudis_30 == "Censored" & tte_icudis_30 == 31))` patients at day 31 who survived the entire 30-day period
but remained in the ICU on day 30.
- We assign the `r with(ptsummary_df, sum(ftype_icudis_30 == "Death"))` patients who died prior to ICU discharge within the 30-day period a time value of `time of death - time of randomization` and a "failure type" of death; we assign the `r with(ptsummary_df, sum(ftype_icudis_30 == "ICU Discharge"))` patients who
were successfully discharged within the 30-day period a time value of `time of discharge - time of randomization` and a "failure type" of ICU discharge.
...
icudis_30_cif <- ggcompetingrisks(
  icudis_30_cif_unadj, conf.int = TRUE, ggtheme = mindusa_theme()
) +
  scale_color_manual(
    name = "",
    values = c(
...
details, please see the [Methods section](#adjusted).
...
looking for generally flat lines in each panel. `r icudis_30_phtext`
...
icudis_30_text <- hr_example(hr_df = icudis_30_hrs, outcome = "ICU discharge")
...
favorable for this outcome. `r icudis_30_text[["A"]]`

TREATMENTS ARE ONCE AGAIN COMPLETELY FAKE

##### Visual Form

```{r icudis_30_hrs_plot, fig.width = 5, fig.height = 3.5}
icudis_30_hrs_plot <- plot_trt_ratios(
...
```{r icudis_30_full_results, warning = FALSE}
rms_results_kable(icudis_30_mod)
